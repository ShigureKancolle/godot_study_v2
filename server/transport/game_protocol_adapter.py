...
class GameProtocolAdapter:
    ...
    def publish_tick_result(self, tick_result: events.TickResult):
        ...
        for event in tick_result.events:
            if isinstance(event, events.CommandRejectedEvent):
                self.publish_command_rejected(event, server_tick)
                continue

            if isinstance(event, events.EntityJoinedEvent):
                connection_id = self._send_join_snapshot(event, server_tick)
                if connection_id:
    ...
    def _send_join_snapshot(self, entity_joined: events.EntityJoinedEvent, server_tick: int):
        logger.debug("实体加入发送世界快照 ：server_tick=%s, account=%s", server_tick, entity_joined.account)
        # 获取连接
        context = self._connections.get_context_by_account_id(entity_joined.account)
        if not context:
            logger.warning("实体加入发送世界快照时找不到连接：account=%s", entity_joined.account)
            return
    ...
    def _make_all_snapshot(self, server_tick: int, room_id: str, self_entity_id: str) -> game_pb2.WorldSnapshot:
        ...
        for entity in self._game_world.get_entities():
            ...
            entity_info = GameProtoProjector.entity_info(info_snapshot)
